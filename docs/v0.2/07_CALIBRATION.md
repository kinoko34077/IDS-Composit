...
初期実装では、`src/calibration/`にpure metric、`tools/calibration/corpus.ts`にverified entryのtrain/holdout分割、`tools/calibration/optimizer.ts`に正規化slotの決定的coordinate search、`tools/calibration/profile.ts`にIDC/role別のevidence集約を置く。実font raster測定とprofile採用判定は、実データを取得してから別Gateとして行う。
