...
現在の入口は、直接IDS入力、候補select、local/CHISE切替、14 spatial IDC、Known Index hit（`⿲彳圭亍 → 街`）、local composition、未対応IDCのsource-preserving fallbackを一覧で確認できる。表のresolver pathは現時点では期待経路の表示であり、CHISE liveの実測結果とは区別する。
